#!/usr/bin/env python3
"""Check or apply Claude Code aliases on a Sub2API OpenAI group."""
...
def main():
    ...
    if current.get("opus_mapped_model") != aliases["claude-opus"]:
        raise RuntimeError("Opus family mapping differs from the desired GPT-6 target")
    if current.get("sonnet_mapped_model") != aliases["claude-sonnet"]:
    ...
    updated["exact_model_mappings"] = {**exact, **changes}
